...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({ children }) => {
	...
	const [darkModeStatus, setDarkModeStatus] = useState(
		localStorage.getItem('dm-xlp4Qd8fb')
			? localStorage.getItem('dm-xlp4Qd8fb') === 'true'
			: process.env.REACT_APP_DARK_MODE === 'true',
	);

	useLayoutEffect(() => {
		localStorage.setItem('dm-xlp4Qd8fb', darkModeStatus.toString());
	}, [darkModeStatus]);
	...
	const [asideStatus, setAsideStatus] = useState(
		localStorage.getItem('asd_aiS899gnp')
			? localStorage.getItem('asd_aiS899gnp') === 'true'
			: // @ts-ignore
			  deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE,
	);
	useLayoutEffect(() => {
		localStorage.setItem('asd_aiS899gnp', asideStatus?.toString());
	}, [asideStatus]);
	...
	useLayoutEffect(() => {
		...
		if (deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE) {
			if (localStorage.getItem('asd_aiS899gnp') === 'true') setAsideStatus(true);
