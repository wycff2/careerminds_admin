...
import CategoryEditModal from './enrollmentEditModal';
...
const CategoryPage = () => {
	const { id } = useParams();
	const itemData = enrollments.filter((item) => item.id.toString() === id?.toString());
	...
	return (
		<PageWrapper className='mt-3' title={dashboardPagesMenu.courseManagement.text} isProtected={true}>
			<SubHeader>
				<SubHeaderLeft>
					<Button
						color='primary'
						...
						icon='ArrowBack'
						tag='a'
						to={`${dashboardPagesMenu.courseManagement.path}`}>
						Back to List
					</Button>
					<SubheaderSeparator />
